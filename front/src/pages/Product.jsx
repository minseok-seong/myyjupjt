...
const Product = () => {
  ...
  const handleContact = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `http://13.124.27.126:5000/api/conversations/single/${id}`
      );
      navigate(`/message/${res.data.id}`);
    } catch (err) {
      if (err.response.status === 404) {
        const res = await axios.post(
          `http://13.124.27.126:5000/api/conversations/`,
          {
            id: id,
            sellerId: sellerId,
            buyerId: buyerId,
  ...
  useEffect(() => {
    const getProduct = async () => {
      try {
        const res = await axios.get(
          `http://13.124.27.126:5000/api/products/find/${id}`
        );
        setproduct(res.data);
      } catch (err) {}
    };
  ...
  return (
    <Container>
      ...
      <Wrapper>
        <ImgContainer>
          <Image src={`http://13.124.27.126:5000/${product.img}`} />
        </ImgContainer>
        <InfoContainer>
          <div
            style={{
